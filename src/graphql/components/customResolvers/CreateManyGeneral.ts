...
import { Ctx, Info, Mutation, Resolver, Arg, ObjectType, Field, Int } from "type-graphql";
import GraphQLJSON from 'graphql-type-json';
import {authorizeTableMutation, authorizeTableMutationData} from "../../auth";
import {GraphQLError} from "graphql";

@ObjectType("CustomAffectedRowsOutput", {})
export class CustomAffectedRowsOutput {
    @Field(_type => Int, {
        nullable: false
    })
    count!: number;
...
export class CreateManyGeneral {
    @Mutation(_returns => CustomAffectedRowsOutput, {
        nullable: false,
    })
    ...
    ): Promise<CustomAffectedRowsOutput> {
        const { _count } = transformInfoIntoPrismaArgs(info);
        const prisma = getPrismaFromContext(ctx);

        if (process.env.USE_TOKEN_AUTH !== '0') {
            if (authorizeTableMutation(model, "create", ctx.permissions) &&
...
async function createRecordsInModel(modelName: string, data: any[], prisma: any): Promise<CustomAffectedRowsOutput> {
    ...
    const results: CustomAffectedRowsOutput = {
        count: 0,
        errorCount: 0,
        records: []
    };
    ...
    if (createData.length > 0) {
        try {
            await prisma.$transaction(async (prismaTransaction: any) => {
                ...
                const largestIdAfterInsert = await prismaTransaction[modelName].findFirst({
                    orderBy: { [primaryKeyField]: 'desc' },
                    select: { [primaryKeyField]: true }
                });
                const largestId = largestIdAfterInsert ? largestIdAfterInsert[primaryKeyField] : 0;
                let currentId = largestId - createData.length + 1;

                createData.forEach((record, index) => {
                    results.records.push({
                        index,
                        data: { ...record, message: `A SUCCESS - Created`, [primaryKeyField]: `${currentId++}` }
                    });
                });
    ...
    const upsertPromises = upsertData.map(async (record, index) => {
        try {
            ...
            const message = createdToday && timeDiff
                ? `SUCCESS - Created`
                : `SUCCESS - Updated`;
            ...
            results.records.push({
                index,
                data: { ...record, message: `ERROR: ${lastLine || "An unknown error occurred"}` }
            });
            results.errorCount++;
        }
    });
...
async function processCreateRowByRow(data: any[], model: any, primaryKeyField: string, results: CustomAffectedRowsOutput) {
    const createPromises = data.map(async (record, index) => {
        try {
            const createdRecord = await model.create({ data: record });
            results.records.push({
                index,
                data: { ...record, message: `SUCCESS - Created`, [primaryKeyField]: `${createdRecord[primaryKeyField]}` }
            });
            results.count++;
        } catch (error: any) {
            const errorLines = error.message.split('\n');
            ...
            results.records.push({
                index,
                data: { ...record, message: `ERROR: ${lastLine || "An unknown error occurred"}`,  [primaryKeyField]: ` `}
            });
            results.errorCount++;
        }
    });
