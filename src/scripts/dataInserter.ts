...
async function insertSchemaData(models: any) {
  ...
  for (const model of models) {
    ...
    const labelField = model.fields.find(
      (field: any) =>
  ...
  for (const model of models) {
    ...
    for (const field of model.fields) {
      if (field.relationFromFields && field.relationFromFields.length > 0) {
        field.relationFromFields.forEach((fieldName: string) =>
  ...
  for (const model of models) {
    ...
    for (const field of model.fields) {
      ...
      const relatedObject =
        field.kind === "object"
          ? crmObjects.get(unCapitalizeFirstLetter(field.type))
          : null;
      ...
      try {
        ...
        console.error(
          `Error processing field ${field.name} for CRMObject ${modelName}:`,
          error,
        );
      }
    }
...
async function insertRelations(models: any) {
  for (const model of models) {
    ...
    try {
      ...
      if (!crmObject) {
        console.warn(
          `CRMObject for model ${modelName} not found. Skipping relations.`,
        );
        continue;
      }
          ...
          try {
            ...
            if (relatedCrmObject) {
              const existingRelation =
                await prisma.cRMObjectRelations.findFirst({
                  where: {
              ...
              if (!existingRelation) {
                const relatedField = models
                  .find(
                    (model: { name: string }) =>
                      model.name === relatedObjectName,
                  )
                  ?.fields.find(
                    (sf: { relationName: string }) =>
                      sf.relationName === field.relationName,
                  );
                ...
                await prisma.cRMObjectRelations.create({
                  data: {
                    primaryObjectId: crmObject.id,
                    relatedObjectId: relatedCrmObject.id,
                    foriegnKeyName: relatedField.relationFromFields[0],
                  },
                });
              }
            }
            ...
            console.error(
              `Error processing relation for CRMObject ${modelName} and field ${field.name}:`,
              error,
            );
          }
        }
      ...
      console.error(
        `Error processing relations for CRMObject ${modelName}:`,
        error,
      );
    }
  }
...
async function cleanUpDeletedEnteries(models: any) {
  try {
    const modelNamesSet = new Set(
      models.map((model: { name: any }) => unCapitalizeFirstLetter(model.name)),
    );
      ...
      if (!modelNamesSet.has(crmObject.name)) {
        ...
        const modelFieldsSet = new Set(
          models
            .find(
              (model: { name: string }) =>
                unCapitalizeFirstLetter(model.name) === crmObject.name,
            )
            .fields.map((field: { name: any }) => field.name),
        );
